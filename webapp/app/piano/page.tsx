...
export default function Piano() {
  ...
  const generate = async () => {
    if (!profile) { setError('Configura prima il tuo profilo.'); return; }
    if (!apiKey) { setError('Configura la API Key nelle Impostazioni.'); return; }
    ...
    try {
      const res = await fetch('/api/piano', {
        ...
        body: JSON.stringify({ profile, genMeal, genWorkout, apiKey }),
      });
      ...
      setPlan(data);
      localStorage.setItem('fitness_plan', JSON.stringify(data));
      setTab('overview');
    } catch (e: unknown) {
    ...
    <div className="space-y-5">
      ...
      <div className="card border-primary/30 bg-primary/5">
        ...
        <div className="space-y-3 mb-4">
          {[
            { label: 'Piano alimentare', val: genMeal, set: setGenMeal },
            { label: 'Piano allenamento', val: genWorkout, set: setGenWorkout },
          ].map(({ label, val, set }) => (
            <div key={label} className="flex items-center justify-between">
              <span className="text-sm font-medium">{label}</span>
              <button
                onClick={() => set(!val)}
                className={`w-12 h-6 rounded-full transition-all relative ${val ? 'bg-primary' : 'bg-border'}`}
              >
        ...
        {!profile && <p className="text-yellow-400 text-xs text-center mt-2">⚠️ Configura prima il profilo</p>}
        {!apiKey && <p className="text-danger text-xs text-center mt-2">⚠️ API Key mancante — vai su Impostazioni</p>}
      </div>
      ...
      {loading && steps.length > 0 && (
        <div className="card">
          ...
          <p className="text-muted text-xs mt-3 italic">La generazione richiede 30-60 secondi...</p>
        </div>
      )}
            ...
            <div className="space-y-4">
              {plan.summary && (
                <div className="card border-accent/30 bg-accent/5">
                  <h3 className="font-bold mb-2">📝 Il tuo piano</h3>
                  <p className="text-sub text-sm leading-relaxed">{plan.summary}</p>
                </div>
              )}
              {(plan.execution_steps ?? []).length > 0 && (
                <div className="card">
                  <h3 className="font-bold mb-3">✅ Agenti eseguiti</h3>
                  {(plan.execution_steps ?? []).map((s, i) => (
                    <p key={i} className="text-sub text-sm mb-1.5">{s}</p>
                  ))}
                </div>
              )}
            </div>
                ...
                <>
                  <div className="card border-accent/30 bg-accent/5">
                    ...
                    <div className="flex items-end gap-1 mb-4">
                      <span className="text-4xl font-extrabold text-accent">{Math.round(meal.total_daily_calories)}</span>
    ...
    <div className="card">
      <button onClick={() => setOpen(!open)} className="w-full flex justify-between items-center">
        ...
        <div className="text-right">
          <p className="text-primary font-bold">{Math.round(meal.total_calories)} kcal</p>
      ...
      {open && (
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex gap-2 mb-3 flex-wrap">
            {[
              { l: 'P', v: meal.total_protein_g },
              { l: 'C', v: meal.total_carbs_g },
              { l: 'F', v: meal.total_fat_g },
            ].map(({ l, v }) => (
              <span key={l} className="bg-surfaceB text-sub text-xs font-semibold px-3 py-1 rounded-full">
                {l}: {Math.round(v)}g
              </span>
            ))}
          </div>
