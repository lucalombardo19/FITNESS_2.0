  ...
  const search = async (query = q) => {
    ...
    try {
      const res = await fetch('/api/cerca', {
        ...
        body: JSON.stringify({ query: query.trim(), dietary, limit: 12 }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setResults(data.results ?? []);
    } catch (e: unknown) {
      setError((e as {message?:string})?.message ?? 'Ricerca fallita');
    ...
    <div className="space-y-5">
      ...
      <div className="card">
        <div className="flex gap-2">
          ...
          <button onClick={() => search()} disabled={!q.trim()||loading} className="btn-primary px-5 flex-shrink-0">
            {loading ? <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg> : '🔍'}
          </button>
        </div>
        <div className="mt-3">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(DIETARY_OPTS).map(([v,l]) => (
              <button key={v} onClick={() => setDietary(prev => prev.includes(v) ? prev.filter(x=>x!==v) : [...prev,v])}
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTIONS.map(s => (
              <button key={s} onClick={() => { setQ(s); search(s); }}
                className="chip bg-surfaceB border-border text-sub hover:border-primary hover:text-primary">{s}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {selected && (
        <div className="card border-accent/40 bg-accent/5">
          ...
          {selected.similarity_score != null && <p className="text-accent text-xs text-right mt-2">Rilevanza: {(selected.similarity_score*100).toFixed(0)}%</p>}
        </div>
      )}

      {results.length > 0 && (
        <>
          <p className="text-sub text-sm">{results.length} risultati trovati — dati USDA certificati</p>
            ...
            {results.map((item,i) => (
              <button key={i} onClick={() => setSelected(item)}
                ...
                <div className="flex justify-between items-start">
                  <div><p className="font-semibold text-sm">{item.name}</p>{item.brand && <p className="text-muted text-xs">{item.brand}</p>}</div>
                  <div className="text-right"><p className="text-primary font-bold">{Math.round(item.calories_per_100g)} kcal</p><p className="text-muted text-xs">/ 100g</p></div>
                </div>
                <div className="flex gap-3 mt-2 text-xs text-sub">
                  ...
                  {item.similarity_score != null && <span className="text-accent ml-auto">{(item.similarity_score*100).toFixed(0)}% match</span>}
                </div>
              </button>
            ))}
          </div>
