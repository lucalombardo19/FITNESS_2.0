...
export default function Profilo() {
  ...
  const save = () => {
    ...
    if (p.fitness_goal === 'cut') kcal -= 400;
    else if (p.fitness_goal === 'bulk') kcal += 300;
    ...
    <div className="space-y-5">
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Età', key: 'age', min: 16, max: 80 },
            { label: 'Peso (kg)', key: 'weight_kg', min: 30, max: 300 },
            { label: 'Altezza (cm)', key: 'height_cm', min: 100, max: 250 },
            ...
            <div key={key}>
              ...
              <input
                ...
                value={p[key as keyof UserProfile] as number}
                min={min} max={max}
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 gap-3">
          {GOALS.map(g => (
            <button
              key={g.value}
              onClick={() => update('fitness_goal', g.value as FitnessGoal)}
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          {ACTIVITY.map(a => (
            <button
              key={a.value}
              onClick={() => update('activity_level', a.value as ActivityLevel)}
      ...
      <div className="card">
        ...
        <div className="flex gap-2">
          {[1,2,3,4,5,6,7].map(n => (
            <button
              key={n}
              onClick={() => update('weekly_workout_frequency', n)}
      ...
      {/* Chip selects */}
      {[
        { title: '🏋️ Attrezzatura', key: 'available_equipment' as const, map: EQUIPMENT },
        { title: '🥗 Preferenze Dietetiche', key: 'dietary_preferences' as const, map: DIETARY },
        { title: '⚠️ Allergie', key: 'allergies' as const, map: ALLERGIES },
      ].map(({ title, key, map }) => (
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(map).map(([val, label]) => {
              ...
              return (
                <button
                  key={val}
                  onClick={() => toggleArr(key, val)}
                  className={`chip ${active ? 'bg-primary/20 border-primary text-primary' : 'bg-surfaceB border-border text-sub hover:border-primary/50'}`}
                >
                  {label}
                </button>
              );
